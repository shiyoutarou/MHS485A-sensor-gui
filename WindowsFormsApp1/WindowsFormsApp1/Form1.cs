...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        {
            pictureBox1.BackgroundImage = Image.FromFile("picture//Grey.png");
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                //按下連線後自動偵測COM，並顯示在textbox1
                string[] comPorts = SerialPort.GetPortNames();
                for (int i = 0; i < comPorts.Length; i++)
                {
                    textBox1.Text = comPorts[i];
                }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            modbus_data[5] = 0x03; //quantity  L
            modbus_data[6] = Convert.ToByte(crc_chk(modbus_data, 6) % 256);  //CRC H
            modbus_data[7] = Convert.ToByte(crc_chk(modbus_data, 6) / 256);  //CRC L
            ...
            byte[] buffer = new byte[11]; //宣告變數格式
            serialPort1.Read(buffer, 0, 11);       //Read COMPORT 

            for (int a = 0; a < 11; a++)
            {
                richTextBox2.AppendText(buffer[a] + " ");
            }
